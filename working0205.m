...
T = 1/fs;             % sample time
f0 = 20E3;            % calibration signal frequency
fc = 868E6;           % carrier frequency


cal_dat = readtable('chamber_0deg_1.csv');
cal_dat = table2array(cal_dat)';

signal = readtable('chamber_60deg_3.csv');
signal = table2array(signal)';


%% %%%%% Upsampling signal
up_calsig = UPsample(cal_dat,10);
% plot(up_calsig(1:400,:))

up_sensig = UPsample(signal,10);
% figure
% plot(up_sensig(1:400,:))

%%%%%%%% Convert signal range 0 - 2V as measured by ADC
up_sensig = (2*up_sensig)/255;         
...
zz = size(up_sensig)
N_samples = zz(1);
[cal_norm_i, cal_norm_q] = norm_cal(up_calsig, freq, N_samples, false);                   
...
Sample_delay = round((10*Phase/f0)/T, 0);

%%%%%%%% Align calibration signals
[al_calsigi, al_calsigq] = sig_align(cal_norm_i, cal_norm_q, Sample_delay);
% plot(al_calsigi(1:300,:))
% title('Aligned Calibration Signals')

%%%%%%%% Normalise real signals
[ni_sig, nq_sig] = norm_cal(up_sensig, freq, 200, false);            
...
SIG_I(min(z_pos_I(1:3)):end, :) = [];
SIG_Q(min(z_pos_Q(1:3)):end, :) = [];
...
function upsig = UPsample(signal,n)

for k=1:8
x(:,k) = 0:1:(size(signal(:,k))-1);       % Original X scale
xq(:,k) = 0:(1/n):(size(signal(:,k))-1);    % Upscaled X scale
temp = spline(x(:,k), signal(:,k), xq);  % upsampled sig
upsig(:,k)= temp(:,k);
end
